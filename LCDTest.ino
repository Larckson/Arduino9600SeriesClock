...
bool Hour24Format = false;bool BuzzerSound = true;int waitTime=500;bool playingMusic = false;

int LeftB = A6;int RightB = A5;int EnterB = A4;int MenuB = A3;int ClearB = A2;int ButtonThreshold = 50;
int ClockClock=12;int DataClock=10;int ResetClock = 7;
int Buzzer = 6;
...
void AssembleString(const RtcDateTime& dt){
  ...
  if(MonthSymbol>9){ MonthSymbol-=10;GreatTen=true; 
  } else {GreatTen=false;}  
  snprintf_P(datestring, countof(datestring),PSTR("%01u%02u"),MonthSymbol,dt.Day());
}

void SetDisplayOut(int Day, char StringDisplay[7], bool AM, char DateDisplay[4]){
  ...
  for (int charDisplay=3;charDisplay>=0;charDisplay--){
    if((StringDisplay[charDisplay]=='0' || StringDisplay[charDisplay]=='O') && charDisplay!=0){
...
void ButtonFunctions(const RtcDateTime& dt){
  ...
  if(analogRead(ClearB)>ButtonThreshold){
    PressButton=true;
    if(!ClearButtonOn){
      ClearButtonOn=true;
